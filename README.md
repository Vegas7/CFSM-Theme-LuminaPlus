...
- **主题设置存在本地，登录站长可发布到后端**：设置页右上角的「保存到本机」把配置存进当前设备的
  ...
  **登录站长**：还多一个「**保存到后端**」——把当前配置直接写到后端，所有设备与访客都会用这套。
  ...
  隐藏（有了「保存到后端」用不到了）。卡片配色面板里的「保存到后端」发的是同一份完整配置。
  站点主题配置里本主题不认识的键（例如内置主题的樱花开关 `mikus`）保存时原样保留，不会被清掉。

  **配色按单个颜色叠**：本机只存改过的那几个颜色，其余跟站点预设走；某个颜色点「恢复」回到站点预设色
  （站点没设才是主题默认色），「全部重置」丢掉本机所有配色覆盖。
  ...
  **后台保存后不是立刻生效，等一两分钟再看**：后端对外观设置有约 2 分钟的内存缓存，而且每个
  Worker 实例各存一份 —— 刚保存完换个浏览器打开，很可能还是旧的一份（主题「保存到后端」的那台设备
  不受影响：保存后两分钟内以刚存的为准）。要确认后端到底下发了什么，
  在站点页面的控制台跑
